create or replace PROCEDURE "UP_CARGA_DIMENSAO_TEMPO"("ano_inicial" IN INT,  "ano_final" IN INT)    AS $$
    DECLARE 
...
-- ** Autor Original Oracle    : GRIMALDO OLIVEIRA
-- ** Autor PostgreSQL: Alfredo Carneiro
...
WHILE ano_inicio <= ano_final loop
    ...
    dtc_dia2 := ((dtr_ano_mes*100) + dia)::text;
   ...
   ELSIF (mes >=9 and mes<11) then
    ...
    SELECT   CASE num_dia_semana
        ...
        IF (mes <10) THEN
       dtc_mes_ano_numerico:='0'||(mes)::text||'/'||substr((dtr_ano_mes)::text,1,4);
    ELSE  
       dtc_mes_ano_numerico:=(mes)::text||'/'||substr((dtr_ano_mes)::text,1,4);
    end if;

        dtc_mes_ano:=substr(nom_mes,1,3)||'/'||substr((dtr_ano_mes)::text,1,4);
        dtc_mes_ano_completo:=nom_mes||'/'||substr((dtr_ano_mes)::text,1,4);
                      
    num_nivel  := 1;
    dtr_ano_mes2 := (dtr_ano_mes)::text;
